...
class DogCarDataset(Dataset):
    ...
    def __init__(self, class_dirs, transform=None):
        ...
        self.samples = []
        self.class_mapping = {"008": "car", "936": "dog"}
...
# ---------------------------
full_dataset = DogCarDataset(CLASS_DIRS, transform=train_transform)
total_images = len(full_dataset)
print("Total images:", total_images)

# Create separate lists of indices for each class.
indices_car = [i for i, (_, label) in enumerate(full_dataset) if label == "car"]
indices_dog = [i for i, (_, label) in enumerate(full_dataset) if label == "dog"]

# We require at least 1300 images per class:
if len(indices_car) < 1300 or len(indices_dog) < 1300:
    raise ValueError("Not enough images per class for stratified splitting!")

random.shuffle(indices_car)
random.shuffle(indices_dog)

# For each class: train = 1000, validation = 200, test = 100
train_indices_car = indices_car[:1000]
val_indices_car   = indices_car[1000:1200]
test_indices_car  = indices_car[1200:1300]

train_indices_dog = indices_dog[:1000]
val_indices_dog   = indices_dog[1000:1200]
test_indices_dog  = indices_dog[1200:1300]

train_indices = train_indices_car + train_indices_dog
val_indices   = val_indices_car   + val_indices_dog
test_indices  = test_indices_car  + test_indices_dog
...
all_aug_labels = []   # e.g., "car_left45"
all_base_labels = []  # e.g., "car"
model.eval()
with torch.no_grad():
...
def plot_tsne_images(image_embeddings, aug_labels):
    ...
    for i, label in enumerate(aug_labels):
        ...
        color = car_colors[aug] if base == "car" else dog_colors[aug]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "normal_dog_car_images.png"), bbox_inches='tight')
    plt.show()
...
def plot_tsne_text(text_embeddings, text_labels):
    ...
    plt.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1, 0.5))
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "normal_dog_car_text.png"), bbox_inches='tight')
    plt.show()
