...
def generate_text_embeddings(clip_model, images, labels):
    clip_model.eval()
    text_embeddings = []
    with torch.no_grad():
        for label in labels:
            text = num_to_word[label]
...
# t-SNE Plotting
def plot_tsne(image_embeddings, text_embeddings, image_labels, text_labels, image_text_embeddings):
    tsne = TSNE(n_components=2, random_state=42)
    
    # Plot for image and text embeddings
    ...
    plt.savefig("meeting-3/tsne_plot_0_1_image")
    plt.show()

    # Plot for image text embeddings
    image_text_tsne = TSNE(n_components=2, random_state=42).fit_transform(image_text_embeddings)
    
    plt.figure(figsize=(12,8))
    for i, label in enumerate(image_labels):
        plt.scatter(*image_text_tsne[i], c=colors[label], marker='o' if '0' in label else '^', s=50, alpha=0.7)
    
    plt.legend(handles=legend_elements[:-2], loc='upper right')
    plt.title("t-SNE of Text Embeddings for Each Image (0s and 1s)")
    plt.xlabel("Dimension 1"), plt.ylabel("Dimension 2")
    plt.savefig("meeting-3/tsne_plot_0_1_image_text")
    plt.show()
    ...
    for digit, data in [(0, x_test_0), (1, x_test_1)]:
        for i in range(50):
            ...
            for transform, shift in [
                ...
                # Generate text embedding for this image
                text = num_to_word[digit]
                seq = torch.tensor(encode(text)).unsqueeze(0).to(device)
                text_emb = clip_model.text_encoder(seq)
                image_text_embeddings.append(text_emb.cpu().numpy().flatten())
...
plot_tsne(np.array(image_embeddings), np.array(text_embeddings), image_labels, text_labels, np.array(image_text_embeddings))