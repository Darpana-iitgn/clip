...
x_train = np.concatenate([x_train_2, x_train_5], axis=0)
y_train = np.concatenate([y_train_2, y_train_5], axis=0)
x_test = np.concatenate([x_test_2, x_test_5], axis=0)
y_test = np.concatenate([y_test_2, y_test_5], axis=0)
...
def plot_tsne(image_embeddings, text_embeddings, image_labels, text_labels):
    ...
    # Image embeddings
    colors = {
        '2_normal': '#1f77b4', '2_left45': '#aec7e8', '2_right45': '#17becf', 
        '2_left135': '#7f7f7f', '2_right135': '#c7c7c7',
        '5_normal': '#d62728', '5_left45': '#ff9896', '5_right45': '#ff7f0e',
        '5_left135': '#8c564b', '5_right135': '#e377c2'
    }
    for i, label in enumerate(image_labels):
        plt.scatter(*embeddings[i], c=colors[label], marker='o' if '2' in label else '^', s=50, alpha=0.7)
    
    # Text embeddings
    text_colors = {'two': '#2ca02c', 'five': '#9467bd'}
    for i, label in enumerate(text_labels, len(image_embeddings)):
        plt.scatter(*embeddings[i], c=text_colors[label], marker='*', s=200)
    
    # Legend
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#1f77b4', markersize=10, label='2 Normal'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#aec7e8', markersize=10, label='2 Left 45°'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#17becf', markersize=10, label='2 Right 45°'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#7f7f7f', markersize=10, label='2 Left 135°'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#c7c7c7', markersize=10, label='2 Right 135°'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='#d62728', markersize=10, label='5 Normal'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='#ff9896', markersize=10, label='5 Left 45°'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='#ff7f0e', markersize=10, label='5 Right 45°'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='#8c564b', markersize=10, label='5 Left 135°'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='#e377c2', markersize=10, label='5 Right 135°'),
        plt.Line2D([0], [0], marker='*', color='w', markerfacecolor='#2ca02c', markersize=15, label='"two"'),
        plt.Line2D([0], [0], marker='*', color='w', markerfacecolor='#9467bd', markersize=15, label='"five"')
    ]
    plt.legend(handles=legend_elements, loc='upper right')
    plt.title("t-SNE of CLIP Embeddings (2s and 5s)")
    plt.xlabel("Dimension 1"), plt.ylabel("Dimension 2")
    plt.savefig("meeting-3/tsne_plot_2_5")
    plt.show()
...
with torch.no_grad():
    for digit, data in [(2, x_test_2), (5, x_test_5)]:
        for i in range(50):
