...
rotate_45_transform = transforms.Compose([
    transforms.Lambda(lambda x: x.squeeze().cpu().numpy()),
    transforms.ToPILImage(),
    transforms.Lambda(lambda img: transforms.functional.rotate(img, angle=45)),
    transforms.ToTensor()
])
...
rotate_135_transform = transforms.Compose([
    transforms.Lambda(lambda x: x.squeeze().cpu().numpy()),
    transforms.ToPILImage(),
    transforms.Lambda(lambda img: transforms.functional.rotate(img, angle=135)),
    transforms.ToTensor()
])
...
class CustomTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx < len(self.dataset) // 3:
            return rotate_45_transform(image), label, text_label
        elif idx < 2 * len(self.dataset) // 3:
            return rotate_135_transform(image), label, text_label
        else:
...
def plot_tsne(image_embeddings, text_embeddings, image_labels, text_labels, title="t-SNE Plot"):
    ...
    colors = {'4': 'blue', '9': 'red'}
    markers = {'normal': 'o', 'rotate45': 's', 'rotate135': '^'}
    for i, label in enumerate(image_labels):
        digit, rotation = label.split('_')
        if digit == '4':
            if rotation == 'normal':
                color = 'blue'
            elif rotation == 'rotate45':
                color = 'red'
            else:  # rotate135
                color = 'green'
        else:  # '9'
        ...
        plt.scatter(reduced_image_embeddings[i, 0], reduced_image_embeddings[i, 1], 
                    c=color, marker=markers[rotation], s=50, alpha=0.7)
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='red', markersize=10, label='4 (45° rotation)'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='green', markersize=10, label='4 (135° rotation)'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='pink', markersize=10, label='9 (normal)'),
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='yellow', markersize=10, label='9 (45° rotation)'),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='magenta', markersize=10, label='9 (135° rotation)'),
    ...
    plt.savefig("meeting-3/tsne_plot_rotate.png")
    plt.show()
...
with torch.no_grad():
    ...
    for digit, x_test in [(4, x_test_4), (9, x_test_9)]:
        for i in range(num_samples):
            ...
            # 45-degree rotation
            emb_45 = clip_model.image_encoder(rotate_45_transform(img).unsqueeze(0).to(device))
            image_embeddings.append(emb_45.cpu().numpy().flatten())
            image_labels.append(f"{digit}_rotate45")
            
            # 135-degree rotation
            emb_135 = clip_model.image_encoder(rotate_135_transform(img).unsqueeze(0).to(device))
            image_embeddings.append(emb_135.cpu().numpy().flatten())
            image_labels.append(f"{digit}_rotate135")
