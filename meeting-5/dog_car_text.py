...
class DogCarDataset(Dataset):
    ...
    def __init__(self, class_dirs, transform=None):
        ...
        self.samples = []
        # Numeric mapping: "008" -> 1 (car), "936" -> 0 (dog)
...
# Define transforms (all images resized to 224x224)
...
# Mapping for base label names (numeric label -> text label)
label_names = {0: "dog", 1: "car"}

# Updated vocabulary including augmented labels
vocab = {
    "dog_left45": 0, "dog_right45": 1, "dog_left135": 2, "dog_right135": 3, "dog_normal": 4,
    "car_left45": 5, "car_right45": 6, "car_left135": 7, "car_right135": 8, "car_normal": 9
}
vocab_size = len(vocab)

def encode_text(text_list):
    """
    Given a list of strings (e.g., ["dog_left45", "car_normal"]), returns a tensor of shape (batch_size, 1).
    """
    indices = [[vocab[t]] for t in text_list]
    return torch.tensor(indices, dtype=torch.long)

# Custom training dataset returns the augmented image and its augmentation-specific text label
class CustomTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Use modulo to choose an augmentation
        mod = idx % 5
...
# Custom test dataset returns the augmented image, augmentation label, and base label
...
# Load full dataset using DogCarDataset
full_dataset = DogCarDataset(CLASS_DIRS, transform=None)

# Separate indices for each class (assume: 0 for dog, 1 for car)
...
# Text Encoder (for single-token texts)
...
def train(epoch):
    model.train()
    total_loss = 0
    for batch_idx, (images, aug_labels, labels) in enumerate(train_loader):
        images = images.to(device)
        # Use the augmentation-specific text labels
        text = encode_text(aug_labels).to(device)
        optimizer.zero_grad()
...
# ----------------------------
# Evaluation and t-SNE Visualization
# ----------------------------

# Extract image embeddings from the test set using augmented text labels
...
all_base_labels = []  # original numeric labels
model.eval()
with torch.no_grad():
    for images, aug_labels, base_labels in tqdm(custom_test_loader, desc="Extracting image embeddings"):
        images = images.to(device)
        # Use the augmented labels to encode text
        text_tokens = torch.stack([encode_text([aug_label]) for aug_label in aug_labels]).to(device)
...
def plot_tsne_images(image_embeddings, aug_labels):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "dog_car_images2.png"), bbox_inches='tight')
    plt.show()

plot_tsne_images(all_image_embeddings, all_aug_labels)

# t-SNE visualization for text embeddings using augmented labels
text_prompts = []
for base in ["car", "dog"]:
    for aug in ["left45", "right45", "left135", "right135", "normal"]:
...
def plot_tsne_text(text_embeddings, text_labels):
    ...
    plt.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1, 0.5))
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "dog_car_text2.png"), bbox_inches='tight')
    plt.show()

plot_tsne_text(text_embeddings, text_prompts)

# Example: Count the number of car samples based on augmented labels
