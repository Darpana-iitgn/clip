...
# "008": car, "936": dog
CLASS_DIRS = {
    "008": os.path.join(DATA_DIR, "n04285008"),
    "936": os.path.join(DATA_DIR, "n02085936")
}

# --- DogCarDataset definition (unchanged except numeric labels) ---
class DogCarDataset(Dataset):
    """
    Dataset for an ImageNet10 structure with two classes:
      - "008": car
      - "936": dog
    Returns numeric labels: 1 for car, 0 for dog.
    """
    def __init__(self, class_dirs, transform=None):
        self.transform = transform
        self.samples = []
        # Numeric mapping to match the rest of the code:
        self.class_mapping = {"008": 1, "936": 0}
...
left_shift_45_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.Lambda(lambda img: transforms.functional.affine(img, angle=45, translate=(-9, 0), scale=1, shear=0)),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406],
                         std=[0.229, 0.224, 0.225])
])
...
left_shift_135_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.Lambda(lambda img: transforms.functional.affine(img, angle=135, translate=(-9, 0), scale=1, shear=0)),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406],
                         std=[0.229, 0.224, 0.225])
])
...
# Use DogCarDataset to load data from the correct folders.
full_dataset = DogCarDataset(CLASS_DIRS, transform=None)

# Separate indices for each class (assuming class 0: dog, class 1: car)
dog_indices = [i for i, (_, label) in enumerate(full_dataset) if label == 0]
car_indices = [i for i, (_, label) in enumerate(full_dataset) if label == 1]
...
train_car = car_indices[:1000]
val_car   = car_indices[1000:1200]
test_car  = car_indices[1200:1300]

train_indices = train_dog + train_car
val_indices   = val_dog + val_car
test_indices  = test_dog + test_car
...
def encode_text(text_list):
    """
    Given a list of strings (e.g., ["dog", "car"]), returns a tensor of shape (batch_size, 1).
    """
...
# all_aug_labels = []   # e.g., "car_left45", "dog_normal", etc.
...
#     car_colors = {"left45": "#8da0cb", "right45": "#1f78b4",
...
#         if base == "car":
#             color = car_colors.get(aug, "#000000")
#         else:
...
#     legend_elements = []
#     for aug, color in car_colors.items():
#         legend_elements.append(plt.Line2D([0], [0], marker='o', color='w', 
#                                           label=f"Car {aug}", markerfacecolor=color, markersize=10))
...
#     plt.tight_layout()
#     plt.savefig(os.path.join(OUTPUT_DIR, "dog_car_images.png"), bbox_inches='tight')
#     plt.show()
...
#                                           label=f"Car {aug}", markerfacecolor=color, markersize=15))
#     for aug, color in dog_colors.items():
#         legend_elements.append(plt.Line2D([0], [0], marker='*', color='w', 
...
#     plt.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1, 0.5))
#     plt.tight_layout()
#     plt.savefig(os.path.join(OUTPUT_DIR, "dog_car_text.png"), bbox_inches='tight')
#     plt.show()

# plot_tsne_text(text_embeddings, text_prompts)

# car_indices = [i for i, label in enumerate(all_aug_labels) if "car" in label]
# print(f"Number of car samples: {len(car_indices)}")
...
def show_augmented_images_grid(full_dataset, normal_transform, left_shift_45_transform, right_shift_45_transform,
                               left_shift_135_transform, right_shift_135_transform, n_per_class=3):  # Changed to 3
    # Find 3 images for each class (dog=0, car=1)
    dog_imgs = []
    car_imgs = []
    for img_path, label in full_dataset.samples:
        if label == 0 and len(dog_imgs) < n_per_class:
            dog_imgs.append(img_path)
        elif label == 1 and len(car_imgs) < n_per_class:
            car_imgs.append(img_path)
        if len(dog_imgs) >= n_per_class and len(car_imgs) >= n_per_class:
            break
    ...
    # 5 rows (augmentations), 6 columns (3 dogs + 3 cars)
    fig, axes = plt.subplots(5, 6, figsize=(15, 15))  # Adjusted columns to 6
    
    for aug_idx, (aug_name, aug_transform) in enumerate(aug_transforms):
        # Process dog images (first 3 columns)
        for i, img_path in enumerate(dog_imgs[:3]):  # Limit to 3
            img = Image.open(img_path).convert("RGB")
            img_tensor = aug_transform(img)
            img_vis = denormalize(img_tensor)
            img_vis = np.transpose(img_vis.numpy(), (1, 2, 0))
            axes[aug_idx, i].imshow(img_vis)
            axes[aug_idx, i].axis('off')
            axes[aug_idx, i].set_title(f"Dog\n{aug_name}", fontsize=8)  # Smaller font
            
        # Process car images (next 3 columns)
        for i, img_path in enumerate(car_imgs[:3]):  # Limit to 3
            img = Image.open(img_path).convert("RGB")
            img_tensor = aug_transform(img)
            img_vis = denormalize(img_tensor)
            img_vis = np.transpose(img_vis.numpy(), (1, 2, 0))
            col_idx = i + 3  # Offset by 3 for car columns
            axes[aug_idx, col_idx].imshow(img_vis)
            axes[aug_idx, col_idx].axis('off')
            axes[aug_idx, col_idx].set_title(f"Car\n{aug_name}", fontsize=8)  # Smaller font

    # Add labels for first row
    for i in range(3):
        axes[0, i].set_title("Dog", fontsize=9, y=1.1)
        axes[0, i+3].set_title("Car", fontsize=9, y=1.1)
    
    plt.subplots_adjust(wspace=0.1, hspace=0.3)
    plt.tight_layout()
    plt.savefig("augmented_grid_3per.png", bbox_inches='tight', dpi=150)
    plt.show()

# Usage (same as before but now shows 3 per class):
show_augmented_images_grid(
    full_dataset,
    normal_transform,
    left_shift_45_transform,
    right_shift_45_transform,
    left_shift_135_transform,
    right_shift_135_transform,
    n_per_class=3  # Explicitly set to 3
)
